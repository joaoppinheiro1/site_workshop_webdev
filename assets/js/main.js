...
});     //basicamente faz os filtros como a própria biblioteca manda
...
//cada elemento deve ter o seu id

// Previously selected topic (if user selected)
const selectedTheme = localStorage.getItem('selected-theme')
const selectedIcon = localStorage.getItem('selected-icon') 

//serve para guardar as ultimas definições deixadas pelo site
...
function imgForward(element) {
    ...
    var imgsArr = Array.from(imgs); 
        /*basicamente demos store das imagens num array*/
    
    imgsArr.every((img, index) => {
        if (img.className.indexOf('hidden')==-1){ /*iterar o array*/
            imgs[index].classList.add('hidden')    /*esconde a imagem seguinte*/
